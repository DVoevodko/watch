...
export const Clock = (props) => {
  const getUserDate = (userTimezone) => {
  ...
  const getInitialState = (userTimezone) => {
  ...
  const [state, setState] = useState(getInitialState(props.userTimezone));
  ...
  return (
    <div className="Clock-box" id={props.id}>
      <h3 className="Clock-name">{props.name}</h3>
      <svg className="Clock" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
        ...
        <g className="Clock-hour" style={{ transform: `rotate(${state.hr}deg)` }}>
          <path className="Clock-hour__hour-arm" d="M300.5 298V142"/>
          <circle className="Clock-sizing-box" cx="300" cy="300" r="253.9"/>
        </g>
        <g className="Clock-minute" style={{ transform: `rotate(${state.min}deg)` }}>
          <path className="Clock-minute__minute-arm" d="M300.5 298V67"/>
          <circle className="Clock-sizing-box" cx="300" cy="300" r="253.9"/>
        </g>
        <g className="Clock-second" style={{ transform: `rotate(${state.sec}deg)` }}>
